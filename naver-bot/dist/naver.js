...
export async function publishNaver(params: {
  ...
  try {
    ...
    // 클립보드 방식으로 입력 (한글 짤림/누락 방지)
    await page.evaluate((t) => {
      const el = document.querySelector(".se-section-documentTitle [contenteditable='true']") as HTMLElement;
      if (el) { el.focus(); document.execCommand("selectAll"); document.execCommand("insertText", false, t); }
    }, title).catch(async () => {
      // fallback: humanType
      await page.keyboard.press("Meta+a");
      await page.waitForTimeout(100);
      await humanType(page, title, { typoRate: 0 }); // 제목은 오타 없이
    });
    await page.waitForTimeout(600);

    // 제목 제대로 들어갔는지 확인 후 재시도
    const titleEntered = await frame.evaluate((t) => {
      const el = document.querySelector(".se-section-documentTitle [contenteditable='true']");
      return el ? el.textContent?.includes(t.slice(0, 5)) ?? false : false;
    }, title).catch(() => false);
    if (!titleEntered) {
      console.log("[naver] 제목 재입력 시도...");
      await frame.click(".se-section-documentTitle", { timeout: 3000 }).catch(() => {});
      await page.waitForTimeout(300);
      await page.keyboard.press("Meta+a");
      await page.waitForTimeout(200);
      await humanType(page, title, { typoRate: 0 });
      await page.waitForTimeout(400);
    }
    ...
    async function insertText(text: string) {
      ...
      for (let i = 0; i < lines.length; i++) {
        if (lines[i].trim()) {
          await humanType(page, lines[i], { typoRate: 0 }); // 본문은 오타 없이 (이미 완성된 글)
          await page.waitForTimeout(100);
        }
        if (i < lines.length - 1) {
          await page.keyboard.press("Enter");
...
export async function generateFlowImages(params: {
  ...
  async function enterPrompt(prompt: string): Promise<boolean> {
    ...
    await page.evaluate(() => {
      ...
      Array.from(document.querySelectorAll("button")).forEach(btn => {
        ...
        if (t.includes("시작") || t.includes("Get started") || t.includes("닫기")) btn.click();
      });
    }).catch(() => {});
    await page.waitForTimeout(800);
    ...
    // 1순위: p[contenteditable] — 최신 Google Flow UI
    try {
      await page.waitForSelector("p[contenteditable='true']", { timeout: 5000 });
      input = await page.$("p[contenteditable='true']");
    } catch {}

    // 2순위: div[contenteditable] (role=textbox 우선)
    if (!input) {
      try {
        input = await page.$("div[contenteditable='true'][role='textbox']");
      } catch {}
    }

    // 3순위: textarea (visible 우선)
    if (!input) {
      try {
        await page.waitForSelector("textarea:visible", { timeout: 4000 });
    ...
    // 5순위: 넓은 contenteditable (combobox 제외)
    if (!input) {
      try {
        input = await page.$("[contenteditable='true']:not([role='combobox']):not([role='listbox'])");
      } catch {}
    }
    ...
    await page.keyboard.press("Backspace");
    await page.waitForTimeout(200);
    await humanType(page, prompt, { typoRate: 0 }); // 프롬프트는 오타 없이
    await page.waitForTimeout(500);
  ...
  try {
    ...
    const dismissPopups = async () => {
      await page.evaluate(() => {
        ...
        btns.forEach(btn => {
          ...
          if (
            t.includes("시작") || t.includes("닫기") || t.includes("동의") ||
            t.includes("started") || t.includes("Get started") ||
            t.includes("Continue") || t.includes("계속") ||
            t.includes("I agree") || t.includes("동의합니다") ||
            t.includes("Accept") || t.includes("확인")
          ) { btn.click(); }
        });
      }).catch(() => {});
      await page.waitForTimeout(1500);
    ...
    // Flow 페이지 완전 로드 대기 헬퍼
    const waitForFlowReady = async () => {
      // networkidle 대기 (최대 10초)
      await page.waitForLoadState("networkidle", { timeout: 10000 }).catch(() => {});
      await page.waitForTimeout(2000);
      await dismissPopups();
      // 추가 팝업/동의 화면 체크 (최대 3번)
      for (let attempt = 0; attempt < 3; attempt++) {
        const hasOverlay = await page.evaluate(() => {
          return !!(
            document.querySelector("[data-state='open']") ||
            document.querySelector("[role='dialog']") ||
            document.querySelector(".consent-bump") ||
    ...
    // 로그인/첫 진입 후 Flow 페이지 완전 안정화
    await waitForFlowReady();
    log("[Flow] ✅ Flow 페이지 준비 완료");
    ...
    for (let i = 0; i < prompts.length; i++) {
      ...
      if (!entered) {
        log(`⚠️ [Flow] ${i + 1}번 프롬프트 입력 실패 — 스킵`);
        continue;
      }
