...
exports.generateFlowImagesCDP = generateFlowImagesCDP;
...
async function downloadImageToTemp(url) {
    try {
        ...
        if (url.startsWith("data:")) {
            ...
            const dtmp = path_1.default.join(os_1.default.tmpdir(), `publy_img_${Date.now()}_${Math.random().toString(36).slice(2, 7)}${dext}`);
            fs_1.default.writeFileSync(dtmp, Buffer.from(m[2], "base64"));
            return dtmp;
        }
...
async function saveNaverSession(userId, id, pw) {
    ...
    try {
        ...
        for (const _sel of ["#loginBtn_row", "#loginBtn_column"]) {
            try {
                const _el = await page.$(_sel);
                if (_el && await _el.isVisible()) {
                    await _el.click();
                    _loginClicked = true;
                    break;
                }
...
async function getNaverCategories(userId) {
    ...
    const page = await context.newPage();
    try {
        await page.goto(`https://blog.naver.com/GoBlogWrite.naver?blogId=${blogId}`, { waitUntil: "domcontentloaded", timeout: 30000 });
        ...
        for (let attempt = 0; attempt < 4 && !panelReady; attempt++) {
            await frame.$$eval("button", (bs) => {
                const b = bs.find(x => (x.textContent || "").trim() === "발행" && /publish_btn/.test(x.className))
                    || bs.find(x => (x.textContent || "").trim() === "발행");
                if (b)
                    b.click();
            }).catch(() => { });
            try {
                await frame.waitForSelector("[class*='option_category']", { timeout: 6000 });
                panelReady = true;
            }
            catch { }
        }
        ...
        await frame.evaluate(() => {
            ...
            const btn = oc?.querySelector("button,[role='button']");
            if (btn)
                btn.click();
        }).catch(() => { });
        await page.waitForTimeout(1500);
...
async function publishNaver(params) {
    ...
    try {
        ...
        async function moveCursorToEnd() {
            ...
            for (const sel of bodySels) {
                try {
                    ...
                    if (els.length) {
                        await els[els.length - 1].click({ timeout: 3000 });
        ...
        async function fillLastImageCaption(text, fromEnd = 0) {
            const cap = (text || "").trim();
            if (!cap)
                return false;
            try {
                ...
                if (imgRes) {
                    await imgRes.click({ force: true }).catch(() => { });
                    await page.waitForTimeout(500);
                }
        ...
        async function linkLastImage(url) {
            const link = (url || "").trim();
            if (!/^https?:\/\//.test(link))
                return false;
            try {
                ...
                const btnClicked = await frame.evaluate(() => {
                    const b = document.querySelector(".se-link-toolbar-button");
                    if (b) {
                        b.click();
                        return true;
                    }
                ...
                let applied = await frame.evaluate(() => {
                    const b = document.querySelector(".se-custom-layer-link-apply-button");
                    if (b) {
                        b.click();
                        return true;
                    }
        ...
        // ★ 마지막 이미지 가운데 정렬
        async function centerLastImage() {
            try {
                const comps = await frame.$$(".se-component.se-image");
                const comp = comps[comps.length - 1];
                if (!comp)
                ...
                await frame.evaluate(() => {
                    const btn = [...document.querySelectorAll("button")].find(b => /가운데 정렬/.test(b.getAttribute("aria-label") || ""));
                    if (btn)
                        btn.click();
                });
                await page.waitForTimeout(300);
        ...
        async function uploadImage(imgUrl, alt, link) {
                    ...
                    if (tmp1 && tmp2) {
                        try {
                            ...
                            if (ok) {
                                ...
                                let capDone = false;
                                if (pairImages[1].alt?.trim())
                                    capDone = await fillLastImageCaption(pairImages[1].alt.trim(), 0) || capDone;
                                if (pairImages[0].alt?.trim())
                                    capDone = await fillLastImageCaption(pairImages[0].alt.trim(), 1) || capDone;
                                // 캡션칸을 못 찾으면 문단 폴백
                                if (!capDone) {
        ...
        if (categoryId) {
            ...
            try {
                ...
                const done = await frame.evaluate((cid) => {
                    const oc = document.querySelector("[class*='option_category']");
                    if (!oc)
                        return false;
                    const radios = Array.from(oc.querySelectorAll("li input[type=radio]"));
                    ...
                    if (!match)
                        return false;
                    const label = oc.querySelector(`label[for="${CSS.escape(match.id)}"]`);
                    (label || match).click();
                    return true;
                }, categoryId);
                await page.waitForTimeout(500);
...
function googleSessionExists(userId) {
    return (0, session_store_1.hasSession)(googleSessionName(userId), LEGACY_SESSION_DIRS);
}
async function saveGoogleSession(userId, email, pw) {
    const browser = await playwright_1.chromium.launch({ headless: false, args: LAUNCH_ARGS });
    const context = await browser.newContext({
    ...
    try {
        ...
        (0, session_store_1.writeSession)(googleSessionName(userId), saved);
        await browser.close();
...
function captionFromPrompt(prompt, idx, total) {
    // 프롬프트에서 "제목" 형태로 키워드 추출
    const titleMatch = prompt.match(/"([^"]+)"/);
    const keyword = titleMatch ? titleMatch[1] : prompt.split(",")[0].replace(/^A\s+\w+\s+\w+\s+\w+\s+/i, "").trim();
    const p = prompt.toLowerCase();
    let style = "";
    if (p.includes("food") || p.includes("gourmet") || p.includes("cuisine"))
        style = "음식 스타일링 사진";
    else if (p.includes("travel") || p.includes("landscape") || p.includes("scenic"))
        style = "여행 풍경 사진";
    else if (p.includes("financial") || p.includes("business") || p.includes("workspace"))
        style = "비즈니스 컨셉 사진";
    else if (p.includes("technology") || p.includes("tech") || p.includes("device"))
        style = "기술 컨셉 사진";
    else if (p.includes("interior") || p.includes("home") || p.includes("decor"))
        style = "인테리어 스타일 사진";
    else if (p.includes("health") || p.includes("fitness") || p.includes("wellness"))
        style = "건강 라이프스타일 사진";
    else if (p.includes("fashion") || p.includes("outfit") || p.includes("style"))
        style = "패션 스타일 사진";
    else if (p.includes("car") || p.includes("vehicle") || p.includes("automotive"))
        style = "자동차 사진";
    else if (p.includes("sport") || p.includes("athlete") || p.includes("action"))
        style = "스포츠 액션 사진";
    else if (p.includes("beauty") || p.includes("skincare") || p.includes("cosmetic"))
        style = "뷰티 사진";
    else if (p.includes("nature") || p.includes("outdoor") || p.includes("garden"))
        style = "자연 풍경 사진";
    else if (p.includes("family") || p.includes("baby") || p.includes("child"))
        style = "가족 사진";
    else if (p.includes("pet") || p.includes("dog") || p.includes("cat"))
        style = "반려동물 사진";
    else if (p.includes("education") || p.includes("study") || p.includes("book"))
        style = "교육 컨셉 사진";
    else if (p.includes("wedding") || p.includes("couple") || p.includes("romance"))
        style = "웨딩 사진";
    else
        style = "관련 사진";
...
async function generateFlowImages(params) {
    ...
    const results = [];
    if (!googleSessionExists(userId))
        throw new Error("Google 세션 없음 — 계정 관리 탭에서 Google 로그인 먼저 해주세요");
    const googleSession = (0, session_store_1.readSession)(googleSessionName(userId), LEGACY_SESSION_DIRS);
    ...
    async function ensureImageMode() {
        ...
        try {
                ...
                try {
                    ...
                    for (const el of els) {
                        ...
                        if (await el.isVisible() && txt.length < 12 && txt.includes("이미지") && !txt.includes("동영상")) {
                            await el.click();
                            await page.waitForTimeout(600);
                            log("  🖼️ [Flow] 출력 모드를 '이미지'로 설정(영상 방지·크레딧 절약)");
                            return true;
                        }
                    }
                }
            ...
            const seen = await page.evaluate(() => {
                const t = [];
                document.querySelectorAll("button,[role='radio'],[role='tab'],[role='option']").forEach(e => { const s = (e.textContent || "").trim(); if (s && s.length < 30)
                    t.push(s); });
                return Array.from(new Set(t)).slice(0, 40);
            }).catch(() => []);
            log(`  ⚠️ [Flow] '이미지' 토글 못 찾음. 화면 버튼들: ${seen.join(" | ")}`);
            return false;
        }
        catch (e) {
            log(`  ⚠️ [Flow] 이미지 모드 설정 중 오류: ${e?.message || e}`);
            return false;
        }
    }
    // ── 헬퍼: 생성된 이미지 저장 ──
    async function saveGeneratedImage(idx, caption) {
        ...
        const saveDataUrlToFile = (dataUrl) => {
            ...
            const fp = path_1.default.join(DOWNLOAD_DIR, `flow_${Date.now()}_${idx}.${ext}`);
            try {
                fs_1.default.writeFileSync(fp, Buffer.from(m[2], "base64"));
                return fp;
            }
            catch {
                return null;
                ...
                const dataUrl = await page.evaluate(async (url) => {
                    try {
                        ...
                        if (!blob || blob.size < 500)
                            return ""; // 너무 작으면 깨진/빈 이미지
                        return await new Promise((resolve) => { const r = new FileReader(); r.onloadend = () => resolve(r.result); r.onerror = () => resolve(""); r.readAsDataURL(blob); });
                    }
                    catch {
                        return "";
                    }
        ...
        for (const src of imgSrcs) {
            try {
                ...
                if (fp) {
                    results.push({ src: fp, alt: caption });
                    log(`  ✅ [Flow] 이미지 ${idx + 1} 저장 완료(퍼블리로 회수)`);
                    return true;
                }
            }
            catch { /* 다음 후보 */ }
        ...
        try {
            ...
            const filePath = path_1.default.join(DOWNLOAD_DIR, `flow_${Date.now()}_${idx}.png`);
...
async function generateFlowImagesCDP(params) {
    ...
    try {
        ...
        backupDir = path_1.default.join(os_1.default.homedir(), "Desktop", `Publy_Flow이미지_${today}`);
    ...
    try {
        ...
        const isFlowUrl = (u) => /labs\.google\/fx|flow\.google\.com/.test(u);
        ...
        //   되어 생성물을 못 본다. 클릭 후 항상 작업 페이지(/project/)를 다시 잡아 page 핸들을 갱신한다.
        {
            ...
            if (proj && proj !== page) {
                ...
                await page.bringToFront().catch(() => { });
                await page.waitForTimeout(800);
                log("[Flow] 🔄 작업 페이지(project)로 전환했어요");
            }
        }
            ...
            try {
                const dump = await page.evaluate(() => {
                    ...
                    const media = {
                        ...
                        imgBig: [...document.querySelectorAll("img")].filter(im => im.naturalWidth >= 300).length,
        ...
        const setOutputCountToOne = async () => {
            ...
            try {
                ...
                if (mult) {
                    await page.waitForTimeout(500);
                    log(`[Flow] ✅ 생성 배수 x1로 설정(1장씩 · ${mult})`);
                    return true;
                }
        ...
        const setImageMode = async () => {
            ...
            try {
                ...
                const isImageNow = async () => page.evaluate(() => {
                    ...
                    if (img) {
                        const a = img.getAttribute("aria-pressed") || img.getAttribute("aria-checked") || img.getAttribute("aria-selected");
                        if (a === "true")
                            return true;
                    }
                    return false;
                ...
                for (let attempt = 0; attempt < 3; attempt++) {
                    ...
                    await page.waitForTimeout(600);
                    if (await isImageNow()) {
                        log("[Flow] 🖼️ 출력 모드를 '이미지'로 설정(영상 방지·크레딧 절약)");
                        return true;
                    }
                ...
                log(`[Flow] ⚠️ 이미지 모드 설정 오류: ${String(e?.message || e).split("\n")[0]}`);
                return false;
            }
        };
        ...
        if (!outputCountIsOne)
            log("[Flow] ℹ️ 1장 설정을 못 걸었어요(무료판이 4장 강제일 수 있음) → 4장 그리드에서 필요한 만큼 재활용해 빠르게 채울게요");
        ...
        while (results.length < target) {
            ...
            if (Date.now() - flowStartedAt > FLOW_MAX_MS) {
                log(`[Flow] ⏱️ ${Math.round(FLOW_MAX_MS / 60000)}분이 지나 여기까지(${results.length}/${target}장)로 마치고 글 발행으로 넘어갈게요`);
                break;
            }
            if (genRuns >= genCap) {
                log(`[Flow] 🧯 생성 시도 상한(${genCap}회) 도달 — ${results.length}/${target}장으로 마치고 글 발행으로 넘어갈게요`);
                break;
            }
            if (queue.length === 0) {
                        ...
                        for (const label of ["새로운 세션", "새 프로젝트", "새 세션", "New session", "New project"]) {
                            try {
                                ...
                                if (await b.count() > 0 && await b.isVisible()) {
                                    ...
                                    log(`[Flow] ↩️ 입력 화면으로 복귀 시도: ${label}`);
                                    await page.waitForTimeout(2000);
                                    break;
                                }
                            }
            ...
            if (!entered) {
                log("[Flow] ⚠️ 입력창을 못 찾음(12초 대기+복귀 시도해도 안 나타남)");
                await dumpFlowControls("입력창 못 찾음");
                requeue();
                await page.waitForTimeout(2500);
                continue;
            }
            ...
            const beforeSrcs = await page.evaluate(() => [...document.querySelectorAll('img')]
                .filter(im => { const el = im; return el.naturalWidth >= 500 && el.naturalHeight >= 500 && /^(https?:|blob:)/.test(el.src || ""); })
                .map(im => im.src));
            ...
            const newImageAppeared = async () => page.evaluate((beforeArr) => {
                const before = new Set(beforeArr);
                return [...document.querySelectorAll('img')]
                    .some(im => { const el = im; return el.naturalWidth >= 500 && el.naturalHeight >= 500 && /^(https?:|blob:)/.test(el.src || "") && !before.has(el.src); });
                ...
                for (let attempt = 0; attempt < 3; attempt++) {
                    const d = await page.evaluate(async (s) => {
                        try {
                            ...
                            const blob = await res.blob();
                            if (!blob || blob.size < 500)
                                return "ERR:tiny"; // 빈/깨진 이미지 방지
                            return await new Promise(r => { const rd = new FileReader(); rd.onloadend = () => r(rd.result); rd.onerror = () => r("ERR:read"); rd.readAsDataURL(blob); });
                        }
                        catch (e) {
            ...
            for (const candidate of freshCandidates) {
                if (harvested >= need)
                    break; // 부족분만큼만 가져오고 나머지는 남겨둠(불필요한 다운로드 방지)
                ...
                if (backupDir) {
                    try {
                        ...
                        const file = path_1.default.join(backupDir, `${safeName}_${harvested}_${ts}.png`);
