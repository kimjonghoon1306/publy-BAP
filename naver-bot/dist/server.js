...
function authMiddleware(req: express.Request, res: express.Response, next: express.NextFunction) {
  if (!BOT_SECRET) return next(); // 시크릿 미설정 시 무시 (하위 호환)
  const secret = req.headers["x-bot-secret"];
  if (secret !== BOT_SECRET) return res.status(401).json({ error: "인증 실패" });
  next();
}

app.use(cors());
app.use(express.json());
// /health는 인증 불필요, 나머지 API는 인증 적용
...
app.post("/api/publish-full", async (req, res) => {
  ...
  try {
    ...
    if (useFlow && flowPrompts?.length > 0 && platform === "naver") {
      ...
      try {
        ...
        if (flowImages.length > 0) {
          ...
          flowWarning = "Flow 이미지 생성은 완료됐으나 저장된 이미지가 없어 이미지 없이 발행됐어요.";
          console.warn("[server] Flow 이미지 0장 — 이미지 없이 발행 계속");
        }
      } catch (flowErr: any) {
        flowWarning = `Flow 이미지 생성 실패 (${flowErr.message}) — 이미지 없이 발행됐어요.`;
    ...
    res.json({ success: true, postUrl, ...(flowWarning ? { warning: flowWarning } : {}) });
  } catch (e: any) {
