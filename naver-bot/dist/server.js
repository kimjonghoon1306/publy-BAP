...
app.post("/api/publish-full", async (req, res) => {
    const { userId, platform, naverId, title, content, pubScope = "full", tags = [], imageUrl, categoryId, visibility, scheduleTime, blocks, videoUrl, videoPosition, editLogNo, editBlogId, useFlow, flowImgCount, flowPrompts, flowCaptions, cdpPort } = req.body;
    ...
    try {
        ...
        if (useFlow && flowPrompts?.length > 0 && platform === "naver") {
            ...
            try {
                ...
                const flowImages = await (0, naver_1.generateFlowImagesCDP)({
                    prompts: flowPrompts,
