...
/* ══════════ 🔍 네이버 서치어드바이저 자동 색인 (소유확인 + 웹페이지 수집요청) ══════════
   봇이 이미 가진 네이버 로그인 세션으로 searchadvisor.naver.com 콘솔에 들어가:
   ① verifySearchAdvisor: 내 블로그가 사이트로 등록(소유확인)돼 있나 확인 + 없으면 등록 시도
   ② requestSearchAdvisorCrawl: 발행한 글 URL을 '웹페이지 수집요청'(하루 50개 제한)
   ⚠️ 로그인 뒤 콘솔 DOM은 실측 전 '추정 셀렉터'다(웹메일 때처럼). 여러 폴백 + 단계별 로그를
      남겨 배포 후 봇 로그를 보고 정확히 교정한다. 추측으로 '된다'고 하지 않는다. */
const SA_BASE = "https://searchadvisor.naver.com";

function saBlogSiteUrl(blogId: string): string { return `https://blog.naver.com/${blogId}/`; }

// 후행 슬래시 보장(네이버 수집요청은 슬래시 누락 시 리디렉션 오류 반환) — 쿼리/해시는 유지
function saNormalizeUrl(url: string): string {
  let u = (url || "").trim();
  if (!u) return u;
  try {
...
// 페이지+모든 프레임 통틀어 첫 매칭 요소 핸들 반환(셀렉터 폴백)
async function saFindFirst(page: any, selectors: string[], timeoutMs = 8000): Promise<any | null> {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    for (const sel of selectors) {
      for (const frame of [page, ...page.frames()]) {
        try { const el = await frame.$(sel); if (el) return el; } catch {}
      }
    }
    await page.waitForTimeout(400);
  }
...
// 소유확인: 콘솔에 내 블로그가 등록돼 있나 확인 + 없으면 등록 시도. 계정연결 시 1회 호출.
export async function verifySearchAdvisor(userId: string, log: (m: string) => void = console.log): Promise<{ ok: boolean; alreadyVerified: boolean; siteUrl: string; message: string }> {
  if (!naverSessionExists(userId)) throw new Error("네이버 세션 없음");
  const { blogId, cookies } = readSession<any>(naverSessionName(userId), LEGACY_SESSION_DIRS);
  const siteUrl = saBlogSiteUrl(blogId);
  log(`[서치어드바이저] 소유확인 시작 — ${siteUrl}`);
  const browser = await chromium.launch({ headless: false, args: LAUNCH_ARGS });   // 소유확인은 visible(사람처럼)
  const context = await browser.newContext({ userAgent: UA, viewport: { width: 1280, height: 900 }, locale: "ko-KR", timezoneId: "Asia/Seoul" });
  await applyAntiDetection(context);
  await context.addCookies(cookies);
  const page = await context.newPage();
  try {
    await page.goto(`${SA_BASE}/console/board`, { waitUntil: "domcontentloaded", timeout: 30000 });
    await page.waitForTimeout(3500);
    if (/nid\.naver\.com|\/login/.test(page.url())) { await browser.close(); return { ok: false, alreadyVerified: false, siteUrl, message: "네이버 로그인 세션 만료 — 계정 재연결 필요" }; }
    // 이미 등록된 사이트 목록에 내 블로그 있나?
    const already = await page.evaluate((bid: string) => document.body.innerText.includes(bid), blogId).catch(() => false);
    if (already) { log(`[서치어드바이저] 이미 등록됨 ✅`); await browser.close(); return { ok: true, alreadyVerified: true, siteUrl, message: "이미 등록됨" }; }
    log(`[서치어드바이저] 미등록 — 사이트 추가 시도(실측 교정 대상)`);
    const input = await saFindFirst(page, ["input[placeholder*='http']", "input[placeholder*='URL']", "input[placeholder*='사이트']", "form input[type='text']", "input[type='url']"]);
    if (input) {
      await input.click().catch(() => {}); await input.fill(siteUrl).catch(async () => { await input.type(siteUrl).catch(() => {}); });
      await page.waitForTimeout(500);
      const added = await page.evaluate(() => {
        const bs = Array.from(document.querySelectorAll("button,a[role='button']")) as HTMLElement[];
        const b = bs.find(x => /추가|등록|확인/.test((x.textContent || "").trim())); if (b) { b.click(); return true; } return false;
      }).catch(() => false);
      log(`[서치어드바이저] 사이트 추가 버튼 클릭=${added}`);
      await page.waitForTimeout(3000);
    } else {
      log(`[서치어드바이저] ⚠️ 사이트 URL 입력칸 못 찾음 — 콘솔 DOM 실측 필요`);
    }
    const nowHas = await page.evaluate((bid: string) => document.body.innerText.includes(bid), blogId).catch(() => false);
    await browser.close();
    return { ok: nowHas, alreadyVerified: false, siteUrl, message: nowHas ? "등록 완료" : "등록 미확인 — 콘솔 실측 교정 필요(네이버 블로그는 소유확인 방식이 다를 수 있음)" };
  } catch (e: any) {
    await browser.close().catch(() => {});
    const msg = String(e?.message || e).split("\n")[0];
    log(`[서치어드바이저] ❌ 소유확인 실패: ${msg}`);
    return { ok: false, alreadyVerified: false, siteUrl, message: msg };
  }
}

// 웹페이지 수집요청: 발행한 글 URL을 색인요청. 발행 성공 직후 호출(일반발행/원터치 공통).
export async function requestSearchAdvisorCrawl(userId: string, url: string, log: (m: string) => void = console.log): Promise<{ ok: boolean; message: string }> {
  if (!naverSessionExists(userId)) throw new Error("네이버 세션 없음");
  const { blogId, cookies } = readSession<any>(naverSessionName(userId), LEGACY_SESSION_DIRS);
  const siteUrl = saBlogSiteUrl(blogId);
  const target = saNormalizeUrl(url);
  log(`[서치어드바이저] 수집요청 — ${target}`);
  const browser = await chromium.launch({ headless: true, args: LAUNCH_ARGS });
  const context = await browser.newContext({ userAgent: UA, viewport: { width: 1280, height: 900 }, locale: "ko-KR", timezoneId: "Asia/Seoul" });
  await applyAntiDetection(context);
  await context.addCookies(cookies);
  const page = await context.newPage();
  try {
    const reqUrl = `${SA_BASE}/console/site/request/crawl?site=${encodeURIComponent(siteUrl)}`;
    await page.goto(reqUrl, { waitUntil: "domcontentloaded", timeout: 30000 });
    await page.waitForTimeout(3500);
    if (/nid\.naver\.com|\/login/.test(page.url())) { await browser.close(); return { ok: false, message: "네이버 로그인 세션 만료 — 계정 재연결 필요" }; }
    const input = await saFindFirst(page, ["input[placeholder*='URL']", "input[placeholder*='url']", "input[placeholder*='주소']", "input[type='url']", "input[type='text']"]);
    if (!input) { await browser.close(); log(`[서치어드바이저] ⚠️ 수집요청 URL칸 못 찾음 — 실측 필요`); return { ok: false, message: "수집요청 입력칸 못 찾음(콘솔 DOM 실측 필요)" }; }
    await input.click().catch(() => {}); await input.fill(target).catch(async () => { await input.type(target).catch(() => {}); });
    await page.waitForTimeout(500);
    const clicked = await page.evaluate(() => {
      const bs = Array.from(document.querySelectorAll("button,a[role='button']")) as HTMLElement[];
      const b = bs.find(x => /확인|요청|수집|제출/.test((x.textContent || "").trim())); if (b) { b.click(); return true; } return false;
    }).catch(() => false);
    await page.waitForTimeout(2500);
    ...
    await browser.close();
    const ok = clicked && !/실패|초과/.test(notice);
    log(`[서치어드바이저] 수집요청 클릭=${clicked} 안내="${notice}"`);
    return { ok, message: notice || (clicked ? "요청 클릭됨(응답 미확인)" : "요청 버튼 못 찾음") };
  } catch (e: any) {
    await browser.close().catch(() => {});
    const msg = String(e?.message || e).split("\n")[0];
    log(`[서치어드바이저] ❌ 수집요청 실패: ${msg}`);
