...
import { deleteAdminPlace360Snapshot, getAdminPlace360DetailUsage, getAdminPlace360Snapshots, Place360DetailUsage, Place360Snapshot, resetAdminPlace360DetailUsage } from "../lib/supabase";
...
export default function Place360AdminManager({ showToast }: Props) {
  ...
  const load = async (query = search) => {
    setLoading(true);
    try { const [nextRows, nextUsage] = await Promise.all([getAdminPlace360Snapshots(query.trim()), getAdminPlace360DetailUsage()]); setRows(nextRows); setUsage(nextUsage); }
