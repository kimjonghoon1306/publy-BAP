...
export const AEO_RULES = `=== ★글 기획(네이버 추천피드·인기글 목표) — 이게 최우선 ===
이 글은 단순 정보 나열이 아니라 '네이버 추천피드·인기글'에 오를 만큼 기획된 글이어야 한다.
독자가 ①첫 2~3줄에서 스크롤을 멈추고(독자의 고민·궁금증을 정확히 찌르거나 의외의 사실·결론을 먼저 던지기) ②끝까지 읽고 ③저장·공유하고 싶게 만든다.
검색하면 다 나오는 뻔한 일반론·사전식 설명은 금지. 실제 겪어본 듯한 구체 정보·수치·팁·비교·주의점만 담고, 문장은 짧고 리듬감 있게(한 문단 2~4문장) 읽는 부담이 없게 쓴다.

=== AI 검색 최적화(AEO) — 네이버 AI 브리핑·Cue: 등 AI 답변에 인용되게(반드시 지킬 것) ===
...
- ★네이버 홈판/인기글 클릭유도(실제 노출 글 분석) — 검색어(SEO)는 유지하면서 아래 후킹 요소를 1개 정도 자연스럽게 섞으면 클릭률↑: ①핵심 표현을 '작은따옴표'로 인용 ②가격·개수·퍼센트 등 구체 숫자 ③결정적 정보를 끝에서 살짝 자르는 말줄임(…) ④"…알고 계셨나요?" 같은 담백한 질문형. ⚠️단 '대박·충격·미쳤다' 스팸 감탄사와 내용과 다른 허위 낚시는 여전히 절대 금지(저품질 필터). 본문에 실제로 있는 내용으로만 후킹.
