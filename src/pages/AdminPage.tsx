          ...
          <div className="main">
            ...
            {tab === "accounts" && (
              <div style={{animation:"fadeUp .25s ease both"}}>
                ...
                {/* ===== 🎨 Google Flow 계정 연결 ===== */}
                <div className="card" style={{padding:"16px 18px",marginTop:4,background:"linear-gradient(135deg,rgba(124,58,237,.08),rgba(168,85,247,.08))",border:"1.5px solid rgba(168,85,247,.3)"}}>
                  ...
                  <div style={{fontSize:11,color:"var(--text)",lineHeight:1.9,marginBottom:12,padding:"10px 12px",borderRadius:10,background:"rgba(168,85,247,.06)",border:"1px solid rgba(168,85,247,.15)"}}>
                    ...
                    <div>① 구글 계정 없으면 먼저 발급받기</div>
                    <div>② 연결하기 클릭 → Chrome 자동 오픈</div>
                    <div>③ Google 로그인 (3분 내 완료)</div>
                  ...
                  <div style={{display:"flex",gap:8}}>
                    ...
                    <button onClick={async()=>{
                      ...
                      if(!window.confirm("Chrome 창이 열립니다. Google 로그인 후 3분 내에 완료해주세요."))return;
                      try{
                        const r=await fetch(`${BOT}/api/google/save-session`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({userId:ADM_UID}),signal:AbortSignal.timeout(200000)});
                        const d=await r.json();
                        d.success?alert("✅ Google Flow 연결 완료! 이제 발행 시 플로우 이미지를 사용할 수 있어요."):alert("❌ 연결 실패: "+(d.error||"다시 시도해주세요"));
                      }catch(e:any){alert("❌ 오류: "+e.message);}
                    }} style={{flex:1,padding:"9px",borderRadius:9,border:"none",background:"linear-gradient(135deg,#7c3aed,#a855f7)",color:"#fff",cursor:"pointer",fontSize:12,fontWeight:800,fontFamily:"inherit"}}>
                      🔗 연결하기
                    </button>
                  </div>
                </div>
