                ...
                <div className="card" style={{padding:"16px 18px",marginTop:4,background:"linear-gradient(135deg,rgba(124,58,237,.08),rgba(168,85,247,.08))",border:"1.5px solid rgba(168,85,247,.3)"}}>
                  ...
                  <div style={{display:"flex",gap:8}}>
                    ...
                    <button onClick={async()=>{
                      ...
                      try{
                        const r=await fetch(`${BOT}/api/google/save-session`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({userId:user?.id}),signal:AbortSignal.timeout(200000)});
                        const d=await r.json();
