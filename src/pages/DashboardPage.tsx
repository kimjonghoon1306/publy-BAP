...
@keyframes stepDone{0%{transform:scale(1)}50%{transform:scale(1.2)}100%{transform:scale(1)}}

/* ── 다크/라이트 테마 ── */
.app.dark{
  --bg:#080c10;--bg2:#0d1117;--card:#111820;--card2:#161d27;--card-hover:#1a2230;
  --border:#1e2836;--border2:#2a3a4f;--border-focus:#00ff9d;
  --text:#e8f4ff;--text2:#7a9ab5;--text3:#4a6478;
  --accent:#00ff9d;--accent-text:#00ff9d;--accent-bg:rgba(0,255,157,.08);--accent-border:rgba(0,255,157,.25);
  ...
  --info:#4ECDC4;--warn:#FFB347;--danger:#FF4757;
  --naver:#03C75A;--tistory:#FF6B35;
  --header-bg:rgba(8,12,16,.96);--shadow:0 4px 24px rgba(0,0,0,.4);
  --right-bg:#0a0f16;
}
.app.light{
  --bg:#f0f4f8;--bg2:#ffffff;--card:#ffffff;--card2:#f8fafc;--card-hover:#f0f4f8;
  --border:#e2e8f0;--border2:#cbd5e0;--border-focus:#00a86b;
  --text:#1a202c;--text2:#4a5568;--text3:#a0aec0;
  --accent:#00a86b;--accent-text:#00a86b;--accent-bg:rgba(0,168,107,.08);--accent-border:rgba(0,168,107,.25);
  --pink:#e91e8c;--pink-bg:rgba(233,30,140,.06);--pink-border:rgba(233,30,140,.2);
  --yellow:#d4a017;--yellow-bg:rgba(212,160,23,.08);--yellow-border:rgba(212,160,23,.25);
  --purple:#6b46c1;--purple-bg:rgba(107,70,193,.08);
  --info:#2c9e97;--warn:#c77800;--danger:#c53030;
  --naver:#03C75A;--tistory:#FF6B35;
  --header-bg:rgba(240,244,248,.97);--shadow:0 2px 12px rgba(0,0,0,.08);
  --right-bg:#f8fafc;
}

/* ── 전체 앱 구조 ── */
.app{display:flex;flex-direction:column;height:100vh;overflow:hidden;background:var(--bg);color:var(--text);font-family:'Noto Sans KR',sans-serif;}

/* ── 헤더 ── */
.header{height:56px;flex-shrink:0;display:flex;align-items:center;padding:0 16px;gap:10px;
  background:var(--header-bg);border-bottom:1px solid var(--border);
  backdrop-filter:blur(24px);position:sticky;top:0;z-index:100;}
.header-logo{font-family:'Space Grotesk',sans-serif;font-size:18px;font-weight:800;
  background:linear-gradient(135deg,var(--accent),#4ECDC4);-webkit-background-clip:text;-webkit-text-fill-color:transparent;}
.header-steps{display:flex;align-items:center;gap:6px;flex:1;justify-content:center;}
.step-dot{width:28px;height:28px;border-radius:50%;display:flex;align-items:center;justify-content:center;
  font-size:11px;font-weight:800;border:2px solid var(--border);color:var(--text3);
  background:var(--bg2);transition:all .3s;cursor:default;flex-shrink:0;}
.step-dot.active{border-color:var(--accent);color:var(--accent);background:var(--accent-bg);animation:pulse 2s infinite;}
.step-dot.done{border-color:var(--accent);background:var(--accent);color:#000;animation:stepDone .3s ease;}
.step-line{width:24px;height:2px;background:var(--border);border-radius:2px;transition:background .3s;}
.step-line.done{background:var(--accent);}
.step-label{font-size:10px;font-weight:700;color:var(--text3);white-space:nowrap;}
.header-right{display:flex;align-items:center;gap:8px;margin-left:auto;}
.plat-badge{display:flex;align-items:center;gap:5px;padding:5px 11px;border-radius:99px;
  font-size:12px;font-weight:700;border:1.5px solid;cursor:pointer;transition:all .15s;flex-shrink:0;}
.plat-badge.naver{border-color:rgba(3,199,90,.4);background:rgba(3,199,90,.08);color:var(--naver);}
.plat-badge.naver.active{background:rgba(3,199,90,.18);border-color:var(--naver);}
.plat-badge.tistory{border-color:rgba(255,107,53,.4);background:rgba(255,107,53,.08);color:var(--tistory);}
.plat-badge.tistory.active{background:rgba(255,107,53,.18);border-color:var(--tistory);}
...
.user-chip{display:flex;align-items:center;gap:7px;padding:5px 12px;border-radius:99px;
  background:var(--card);border:1px solid var(--border);cursor:pointer;
  font-size:12px;font-weight:600;color:var(--text);transition:all .15s;max-width:130px;}
...
/* ── 좌측 네비 ── */
.left-nav{width:180px;flex-shrink:0;background:var(--bg2);border-right:1px solid var(--border);
...
.nav-item{display:flex;align-items:center;gap:10px;padding:11px 12px;border-radius:10px;
  ...
  font-family:'Noto Sans KR',sans-serif;color:var(--text2);background:transparent;
...
.nav-badge{margin-left:auto;font-size:10px;font-weight:800;padding:2px 7px;
  border-radius:99px;background:var(--accent-bg);color:var(--accent-text);border:1px solid var(--accent-border);}
.nav-footer{margin-top:auto;padding:10px 6px 4px;border-top:1px solid var(--border);}
.quota-mini{padding:10px 12px;border-radius:10px;background:var(--card);border:1px solid var(--border);}
.quota-mini-label{font-size:10px;color:var(--text3);font-weight:700;margin-bottom:6px;letter-spacing:.08em;}
.quota-bar{height:5px;border-radius:99px;background:var(--border);overflow:hidden;margin-bottom:5px;}
.quota-fill{height:100%;border-radius:99px;background:var(--accent);transition:width .5s;}
.quota-count{font-size:12px;font-weight:800;color:var(--accent-text);}

/* ── 메인 컨텐츠 ── */
.main-content{flex:1;overflow-y:auto;padding:18px 20px;min-width:0;}
.main-content::-webkit-scrollbar{width:4px;}
.main-content::-webkit-scrollbar-thumb{background:var(--border2);border-radius:99px;}

/* ── 우측 패널 (항상 고정) ── */
.right-panel{width:280px;flex-shrink:0;background:var(--right-bg);border-left:1px solid var(--border);
...
.rp-section{background:var(--card);border:1px solid var(--border);border-radius:12px;padding:12px 14px;}
.rp-title{font-size:10px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;
  color:var(--text3);margin-bottom:10px;display:flex;align-items:center;gap:6px;}
.rp-row{display:flex;align-items:flex-start;justify-content:space-between;gap:8px;
  padding:5px 0;border-bottom:1px solid var(--border);font-size:12px;}
.rp-row:last-child{border-bottom:none;}
.rp-key{color:var(--text3);font-weight:600;white-space:nowrap;flex-shrink:0;}
.rp-val{color:var(--text);font-weight:700;text-align:right;word-break:break-all;font-size:11px;}
.rp-val.accent{color:var(--accent-text);}
.rp-val.warn{color:var(--warn);}
.rp-thumb{width:100%;aspect-ratio:16/9;border-radius:8px;object-fit:cover;
  border:1px solid var(--border);background:var(--bg2);}
.rp-btn{width:100%;padding:11px;border-radius:10px;border:none;cursor:pointer;
  font-size:13px;font-weight:800;font-family:'Noto Sans KR',sans-serif;
  transition:all .15s;display:flex;align-items:center;justify-content:center;gap:7px;}
.rp-btn-primary{background:var(--accent);color:#000;}
...
.rp-btn-secondary:hover{border-color:var(--border2);}
.pattern-chips{display:flex;gap:6px;}
.pattern-chip{flex:1;padding:6px;border-radius:8px;border:1.5px solid var(--border);
  background:transparent;cursor:pointer;font-size:11px;font-weight:700;
  color:var(--text2);text-align:center;transition:all .15s;}
.pattern-chip.active{border-color:var(--accent);background:var(--accent-bg);color:var(--accent-text);}
...
.card-title{font-size:11px;font-weight:800;letter-spacing:.1em;text-transform:uppercase;
  color:var(--text2);display:flex;align-items:center;gap:6px;}

/* ── 입력 ── */
.inp{width:100%;padding:10px 13px;border-radius:10px;border:1.5px solid var(--border);
  background:var(--bg2);color:var(--text);font-size:14px;font-family:'Noto Sans KR',sans-serif;
  transition:border-color .15s;outline:none;}
.inp:focus{border-color:var(--border-focus);}
.inp.lg{font-size:15px;padding:12px 14px;}
.inp-label{display:block;font-size:11px;font-weight:700;color:var(--text2);margin-bottom:7px;letter-spacing:.05em;}

/* ── 버튼 ── */
.btn{padding:9px 18px;border-radius:10px;border:none;cursor:pointer;font-size:13px;
  font-weight:700;font-family:'Noto Sans KR',sans-serif;transition:all .15s;
...
.btn-xl{padding:14px;font-size:15px;border-radius:12px;}

/* ── STEP 진행바 (탭 내부) ── */
.steps{display:flex;border-radius:12px;overflow:hidden;border:1px solid var(--border);margin-bottom:18px;background:var(--bg2);}
.step-item{flex:1;padding:12px 8px;text-align:center;font-size:11px;font-weight:700;
  color:var(--text3);border-right:1px solid var(--border);transition:all .2s;position:relative;}
.step-item:last-child{border-right:none;}
.step-item.active{color:var(--accent-text);background:var(--accent-bg);}
.step-item.done{color:var(--text2);}
.step-item.done::after{content:'✓';position:absolute;top:6px;right:8px;font-size:10px;color:var(--accent);}
.step-n{display:block;font-size:10px;opacity:.6;margin-bottom:2px;}

/* ── 제목 카드 ── */
.title-card{padding:13px 15px;border-radius:11px;border:1.5px solid var(--border);
  background:var(--bg);cursor:pointer;text-align:left;font-family:'Noto Sans KR',sans-serif;
...
.title-chk{position:absolute;top:10px;right:12px;width:20px;height:20px;border-radius:50%;
  background:var(--accent);color:#000;font-size:11px;font-weight:900;
  display:flex;align-items:center;justify-content:center;}

/* ── 선택된 제목 배너 ── */
.sel-banner{padding:13px 16px;border-radius:12px;background:var(--accent-bg);
  border:1.5px solid var(--accent-border);margin-bottom:14px;}
.sel-banner-lbl{font-size:10px;font-weight:800;color:var(--accent-text);letter-spacing:.08em;margin-bottom:5px;}
.sel-banner-txt{font-size:14px;font-weight:700;color:var(--text);}

/* ── 플로우 버튼 ── */
.flow-nav{display:flex;align-items:center;justify-content:center;gap:10px;margin:18px 0 4px;flex-wrap:wrap;}
.flow-btn{padding:11px 22px;border-radius:10px;border:none;cursor:pointer;font-size:13px;font-weight:800;
  font-family:'Noto Sans KR',sans-serif;transition:all .15s;display:flex;align-items:center;gap:8px;}
.flow-btn-g{background:var(--accent);color:#000;}
.flow-btn-g:hover{opacity:.88;transform:translateY(-1px);}
...
.img-item{position:relative;border-radius:10px;overflow:hidden;aspect-ratio:16/9;
  border:2px solid transparent;cursor:pointer;transition:all .15s;background:var(--bg2);}
.img-item:hover{border-color:var(--border2);}
.img-item.thumb{border-color:var(--accent);}
.img-item img{width:100%;height:100%;object-fit:cover;}
.img-thumb-badge{position:absolute;top:6px;left:6px;background:var(--accent);color:#000;
  font-size:10px;font-weight:800;padding:2px 8px;border-radius:99px;}
.img-del{position:absolute;top:6px;right:6px;width:22px;height:22px;border-radius:50%;
  background:rgba(0,0,0,.7);border:none;color:#fff;cursor:pointer;font-size:13px;
  display:flex;align-items:center;justify-content:center;}

/* ── 이미지 캡션 입력 ── */
.img-caption-wrap{margin-top:4px;}
.img-caption-inp{width:100%;padding:5px 8px;border-radius:7px;border:1px solid var(--border);
  background:var(--bg2);color:var(--text);font-size:11px;font-family:'Noto Sans KR',sans-serif;outline:none;}
.img-caption-inp:focus{border-color:var(--border-focus);}
.img-caption-inp::placeholder{color:var(--text3);}

/* ── 수량/영상 설정 ── */
.media-setting{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px;}
.media-card{padding:12px 14px;border-radius:12px;background:var(--card2);border:1px solid var(--border);}
.media-label{font-size:10px;font-weight:800;color:var(--text3);letter-spacing:.08em;margin-bottom:8px;}
.media-count-row{display:flex;align-items:center;gap:8px;}
.media-count-btn{width:28px;height:28px;border-radius:8px;border:1px solid var(--border);
  background:var(--bg2);color:var(--text);cursor:pointer;font-size:16px;font-weight:700;
  display:flex;align-items:center;justify-content:center;transition:all .15s;}
.media-count-btn:hover{background:var(--card-hover);}
.media-count-val{font-size:18px;font-weight:900;color:var(--accent-text);font-family:'Space Grotesk',sans-serif;min-width:28px;text-align:center;}
.media-mode{display:flex;gap:6px;margin-bottom:8px;}
.media-mode-btn{flex:1;padding:5px;border-radius:7px;border:1.5px solid var(--border);
  background:transparent;font-size:11px;font-weight:700;color:var(--text2);cursor:pointer;transition:all .15s;}
.media-mode-btn.active{border-color:var(--accent);background:var(--accent-bg);color:var(--accent-text);}
.video-toggle{display:flex;align-items:center;justify-content:space-between;}
.toggle-sw{width:40px;height:22px;border-radius:99px;background:var(--border);border:none;
...
/* ── 알림 박스 ── */
.alert-box{padding:12px 16px;border-radius:11px;margin-bottom:12px;font-size:13px;font-weight:600;display:flex;align-items:center;gap:10px;}
.alert-warn{background:rgba(255,179,71,.07);border:1px solid rgba(255,179,71,.25);color:var(--warn);}
.alert-danger{background:rgba(255,71,87,.07);border:1px solid rgba(255,71,87,.25);color:var(--danger);}
.alert-success{background:var(--accent-bg);border:1px solid var(--accent-border);color:var(--accent-text);}

/* ── 수익화 타입 ── */
.adtype-row{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:14px;}
.adtype-btn{padding:14px 12px;border-radius:12px;border:2px solid var(--border);
  background:var(--bg);cursor:pointer;text-align:left;font-family:'Noto Sans KR',sans-serif;
  transition:all .15s;position:relative;}
.adtype-btn:hover{border-color:var(--border2);}
.adtype-lbl{font-size:13px;font-weight:800;color:var(--text);margin-bottom:4px;}
.adtype-sub{font-size:11px;color:var(--text2);line-height:1.5;}
.adtype-btn.sel-adpost{border-color:var(--naver);background:rgba(3,199,90,.07);}
.adtype-btn.sel-adsense{border-color:var(--info);background:rgba(78,205,196,.07);}

/* ── 발행 컨셉 ── */
.concept-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:14px;}
.concept-btn{padding:14px 10px;border-radius:12px;border:2px solid var(--border);
  background:var(--bg);cursor:pointer;text-align:center;font-family:'Noto Sans KR',sans-serif;transition:all .15s;}
.concept-btn:hover{border-color:var(--border2);}
.concept-ico{font-size:22px;margin-bottom:6px;}
.concept-name{font-size:12px;font-weight:800;color:var(--text);margin-bottom:4px;}
.concept-sub{font-size:10px;color:var(--text2);line-height:1.4;white-space:pre-line;}
...
/* ── 계정 카드 ── */
.acc-card{padding:14px 16px;border-radius:12px;background:var(--card2);
  border:1.5px solid var(--border);margin-bottom:10px;display:flex;align-items:center;gap:12px;
  animation:fadeUp .25s ease both;}
.acc-card.conn-naver{border-color:rgba(3,199,90,.35);}
.acc-card.conn-tistory{border-color:rgba(255,107,53,.35);}

/* ── 히스토리 ── */
.hist-item{padding:13px 16px;border-radius:12px;background:var(--card2);
  border:1px solid var(--border);margin-bottom:8px;animation:fadeUp .2s ease both;}
.hist-meta{font-size:11px;color:var(--text3);}

/* ── 스피너 ── */
.spinner{width:16px;height:16px;border-radius:50%;border:2px solid rgba(0,0,0,.2);
  border-top-color:currentColor;animation:spin .8s linear infinite;flex-shrink:0;}
.btn-stop{padding:7px 16px;border-radius:99px;border:1.5px solid var(--danger);
  background:transparent;color:var(--danger);cursor:pointer;font-size:12px;font-weight:700;font-family:'Noto Sans KR',sans-serif;}

/* ── 미리보기 ── */
.preview-overlay{position:fixed;inset:0;z-index:500;background:rgba(0,0,0,.92);
  display:flex;align-items:flex-start;justify-content:center;overflow-y:auto;}
.preview-inner{width:100%;max-width:860px;min-height:100vh;background:#fff;
  padding:48px 40px;animation:guideIn .3s ease both;margin:0 auto;}

/* ── 가이드 모달 ── */
.guide-overlay{position:fixed;inset:0;z-index:999;background:rgba(0,0,0,.78);
  backdrop-filter:blur(8px);display:flex;align-items:center;justify-content:center;padding:12px;}
.guide-modal{width:100%;max-width:560px;max-height:88vh;border-radius:20px;
  background:#0a1520;border:1px solid rgba(0,255,157,.15);display:flex;flex-direction:column;overflow:hidden;}
.guide-header{padding:18px 20px 0;border-bottom:1px solid rgba(255,255,255,.06);}
.guide-tabs{display:flex;gap:0;overflow-x:auto;scrollbar-width:none;}
.guide-tabs::-webkit-scrollbar{display:none;}
.guide-tab{padding:11px 15px;border:none;background:transparent;font-size:12px;font-weight:700;
  color:rgba(255,255,255,.4);cursor:pointer;font-family:'Noto Sans KR',sans-serif;
  white-space:nowrap;border-bottom:3px solid transparent;transition:all .15s;flex-shrink:0;}
.guide-tab.active{color:#FFD93D;border-bottom-color:#FFD93D;}
.guide-body{flex:1;overflow-y:auto;background:#0d1a0d;padding:18px 20px 22px;}
.guide-footer{padding:12px 16px;border-top:1px solid rgba(255,255,255,.06);
  display:flex;justify-content:flex-end;gap:8px;}

/* ── 하단 모바일 탭바 ── */
.mob-bar{display:none;position:fixed;bottom:0;left:0;right:0;z-index:200;
  background:var(--header-bg);border-top:1px solid var(--border);backdrop-filter:blur(24px);
  padding:6px 4px max(12px,env(safe-area-inset-bottom));}
.mob-tab{flex:1;display:flex;flex-direction:column;align-items:center;gap:2px;
  ...
  font-family:'Noto Sans KR',sans-serif;transition:all .15s;min-height:50px;}
.mob-tab-ico{font-size:22px;}
.mob-tab-lbl{font-size:10px;font-weight:700;color:var(--text3);}
...
@media(max-width:600px){
  ...
  .adtype-row{grid-template-columns:1fr;}
  .media-setting{grid-template-columns:1fr;}
  .img-grid{grid-template-columns:repeat(2,1fr);}
  .preview-inner{padding:24px 16px;}
}
...
export default function DashboardPage({user, onLogout, onAdminLogin, onThemeToggle, theme}: Props) {
  ...
  // ─── 이미지 프롬프트는 buildImgPrompt 함수에서 동적 생성 ───

  function buildImgPrompt(kw: string, title: string = "", idx: number = 0): string {
    const k = (kw + " " + title).toLowerCase();
    const base = "photorealistic, ultra detailed, 8K resolution, professional photography, no people, no person, no face, no human";
    // 조명 변주 (이미지마다 다른 분위기)
    const lighting = ["natural soft lighting","golden hour warm light","dramatic studio lighting","bright clean daylight"][idx % 4];

    // 음식/맛집/카페
    if (/맛집|음식|요리|식당|레스토랑|먹|카페|디저트|빵|과자|반찬|국|찌개|탕|라면|치킨|피자|햄버거|초밥|회|굴|조개|해산물|굴비|생선|건어물|황태|북어|젓갈|김치|된장|고추장/.test(k))
      return `Korean food dish, close-up food photography, delicious presentation, styled plating, ${lighting}, bokeh background, appetizing, food styling magazine quality, ${base}`;

    // 카페/커피/음료
    if (/카페|커피|아메리카노|라떼|에스프레소|음료|차|tea|juice/.test(k))
      return `Korean cafe interior, beautiful latte art in ceramic cup, cozy ambiance, ${lighting}, minimal aesthetic, coffee shop photography, ${base}`;

    // 여행/관광/지역
    if (/여행|관광|명소|여행지|해외|국내|제주|부산|서울|강원|경주|전주|속초|tour|travel|destination|trip/.test(k))
      return `beautiful Korean travel destination, stunning scenic landscape, ${lighting}, vibrant colors, wide angle photography, National Geographic style, ${base}`;

    // 건강/다이어트/운동
    if (/건강|운동|다이어트|헬스|피트니스|영양|비타민|의학|병원|약|피부|탈모|다이어트|wellness|health|fitness|diet/.test(k))
      return `healthy lifestyle concept, nutritious food ingredients, fresh vegetables, wellness product, ${lighting}, clean white background, health photography, ${base}`;

    // 재테크/금융/투자
    if (/재테크|투자|주식|부동산|코인|적금|보험|금융|돈|수익|절약|경제|finance|investment|money|stock/.test(k))
      return `financial success concept, modern business aesthetic, growth chart, coins and bills flat lay, ${lighting}, corporate minimal style, ${base}`;

    // IT/기술/AI/앱
    if (/it|기술|ai|인공지능|앱|소프트웨어|코딩|개발|스마트폰|노트북|컴퓨터|tech|software|digital|app/.test(k))
      return `modern technology concept, futuristic digital background, laptop and devices, ${lighting}, blue neon glow, high tech aesthetic, clean minimal, ${base}`;

    // 뷰티/패션/스타일
    if (/뷰티|화장|패션|스타일|옷|코디|메이크업|beauty|fashion|makeup|cosmetic|style/.test(k))
      return `beauty and fashion product photography, elegant composition, ${lighting}, high fashion aesthetic, studio backdrop, luxury feel, ${base}`;

    // 육아/아이/가족
    if (/육아|아이|어린이|유아|임신|출산|가족|엄마|아빠|parenting|child|family|baby|kids/.test(k))
      return `warm family lifestyle concept, cozy home setting, soft pastel tones, ${lighting}, heartwarming atmosphere, ${base}`;

    // 부동산/인테리어/집
    if (/부동산|인테리어|집|아파트|원룸|거실|주방|침실|real estate|interior|home|house|apartment/.test(k))
      return `luxury interior design photography, modern Korean home, bright spacious room, architectural photography, ${lighting}, Scandinavian minimal style, ${base}`;

    // 자동차
    if (/자동차|차|드라이브|자동차|car|vehicle|driving|automotive/.test(k))
      return `professional automotive photography, sleek car design, dramatic lighting, motion blur background, ${lighting}, ${base}`;

    // 교육/공부/자격증
    if (/교육|공부|학습|시험|자격증|학교|수능|study|education|learning|exam|school/.test(k))
      return `education concept photography, open books and stationery flat lay, study desk, ${lighting}, motivating bright atmosphere, ${base}`;

    // 반려동물
    if (/강아지|고양이|반려|펫|dog|cat|pet|animal/.test(k))
      return `adorable pet photography, soft bokeh background, ${lighting}, cute heartwarming product, ${base}`;

    // 자연/계절/환경
    if (/자연|환경|숲|바다|산|꽃|봄|여름|가을|겨울|nature|forest|ocean|mountain|flower|season/.test(k))
      return `stunning Korean nature landscape, ${lighting}, vibrant scenery, National Geographic photography style, ${base}`;

    // 기본 fallback
    return `Korean lifestyle blog thumbnail, clean professional composition, ${lighting}, high quality stock photo style, bright engaging visual, ${base}`;
  }
...
[관련글끝]`;
    try{
      ...
      const full=bm?bm[1].trim():cleaned;
      // 본문 / FAQ / 관련글 분리
      const faqIdx=full.indexOf("[FAQ시작]");
      const bodyOnly=faqIdx>0?full.slice(0,faqIdx).trim():full;
      // genContent는 full(본문+FAQ+관련글) 저장 → buildPublishContent로 분리
      setGenContent(full);
      if(imgCountAuto)setImgCount(recommendImgCount(bodyOnly));
  ...
  const connAccs=accounts.filter(a=>a.platform===platform);
          ...
          <div className="main">
                  ...
                  <div className="card" style={{animation:"fadeUp .2s ease both"}}>
                    <div className="card-header">
                      ...
                      <div style={{display:"flex",gap:7,alignItems:"center"}}>
                        <span style={{padding:"4px 12px",borderRadius:99,fontSize:12,fontWeight:800,background:"var(--accent-bg)",color:"var(--accent-text)",border:"1px solid var(--accent-border)"}}>{(()=>{const i=genContent.indexOf("[FAQ시작]");return(i>0?genContent.slice(0,i).trim():genContent).length.toLocaleString()})()}자</span>
                    ...
                    <div style={{display:"flex",flexDirection:"column",gap:12,marginBottom:14}}>
                      ...
                      <div>
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><label className="inp-label" style={{margin:0}}>본문</label><span style={{fontSize:12,color:"var(--text2)"}}>{(()=>{const i=genContent.indexOf("[FAQ시작]");return(i>0?genContent.slice(0,i).trim():genContent).length.toLocaleString()})()}자</span></div>
            ...
            {tab==="publish"&&(
              <div style={{animation:"fadeUp .25s ease both"}}>
                ...
                {!genContent&&(
                  <div className="alert-box alert-warn" style={{display:"flex",alignItems:"center",gap:10}}>
                    ⚠️ 먼저 글 생성 탭에서 글을 만들어주세요
                    <button className="btn btn-secondary btn-sm" style={{marginLeft:"auto",flexShrink:0}} onClick={()=>setTab("write")}>✍️ 글 생성하러 가기 →</button>
                  </div>
                )}
